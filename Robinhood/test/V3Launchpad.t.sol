...
abstract contract V3LaunchpadTestBase is Test {
    ...
    function test_transferCreator_redirectsWethFees() public {
        V3Launchpad.LaunchInfo memory info = pad.getLaunch(token);
        vm.prank(creator);
        pad.transferCreator(token, alice);
        ...
        locker.claim(token);
        assertEq(weth.balanceOf(alice), 0.5 ether, "new dev receives the WETH share");
