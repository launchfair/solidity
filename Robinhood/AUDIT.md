...
| Graduation target owner-settable | `setGraduationWethAmount` is owner-only and rejects 0. It is global and applies to existing tokens' graduation/`curveProgress` retroactively — a deliberate manual knob (centralization item C-02 below), not a fund-moving power. |
...
3. **C-02 — owner can move the graduation target for all tokens.** `setGraduationWethAmount` is a single global, owner-only knob applied retroactively (a deliberate feature). It cannot move funds or affect the locked liquidity — worst case it flips a token's cosmetic "bonded" flag / progress bar. If even that should be trustless, cap it or snapshot per token at creation; otherwise keep the owner behind a multisig.
