...
///
/// IMMUTABLE variant: the fee rate, split, and destinations are constructor immutables with NO
/// owner and NO setters, so a token scanner sees no admin surface. Fees accrue in the STOCK; the
/// PERMISSIONLESS `claimFees(token)` (self-quoted min-out floor, TWAP-guarded so it can't be
/// sandwiched) redeems the claims, converts stock→WETH on Uniswap V3 using the StockPairRouter's own
/// route config (`sellPathOf`, falling back to the launchpad's `quoteV3Fee` direct hop), then pays
/// the flat 4-way split in native ETH: dev 50 / mechanism 30 / treasury 10 / buyback 10. A Base
/// stock token has no mechanism, so its 30% folds into the buyback (40% to the flywheel); a mode
/// stock token (Reward/Lottery) funds its own mechanism with the 30%.
///
...
contract StockFeeHookImmutable is IHooks {
    ...
    uint16 public immutable feeBps; // fee on the quote (stock) leg, both directions
    ...
    /// V3 factory for the permissionless claim's self-quoted floor. Set at construction.
    IUniswapV3Factory public immutable claimV3Factory;
    ...
    uint16 public immutable claimSlippageBps;
    ...
    constructor(
        ...
        IUniswapV3Factory claimV3Factory_,
        ...
        claimV3Factory = claimV3Factory_;
        claimSlippageBps = claimSlippageBps_;
    ...
    /// `claimSlippageBps`), same defense as FlagshipBuyback.buyback(). The claim config (v3Factory)
    /// is fixed at construction; if it was deployed unset, claimFees reverts NotConfigured.
